...
if ! mysql -h $MYSQL_SLAVE_HOST -e "use ESC4;"; then
  mysql -h $MYSQL_SLAVE_HOST -e "STOP SLAVE;CHANGE REPLICATION FILTER REPLICATE_IGNORE_DB = (training_ESC4);CHANGE MASTER TO MASTER_HOST='mysql', MASTER_USER='repl',MASTER_PASSWORD='$MYSQL_SLAVE_PW',MASTER_LOG_FILE='master-bin.000001',MASTER_LOG_POS=4;START SLAVE;"
fi
